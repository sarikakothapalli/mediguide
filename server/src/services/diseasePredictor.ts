import modelData from '../data/symptomDiseaseModel.json' with { type: 'json' };
...
function normalize(value: string): string {
  return value.trim().toLowerCase().replace(/\s+/g, '_').replace(/_+/g, '_').replace(/\.\d+$/, '');
}
...
export function predictDiseases(symptoms: string[], topK = 3): DiseasePredictionResult {
  ...
  const scores = modelData.coefficients.map((row, classIndex) => {
    ...
    for (let featureIndex = 0; featureIndex < modelData.features.length; featureIndex += 1) {
      if (activeFeatures.has(modelData.features[featureIndex])) score += row[featureIndex] ?? 0;
    }
    return score;
  });
  const predictions = softmax(scores)
